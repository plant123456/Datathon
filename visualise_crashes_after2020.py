...
crashes = pd.read_csv('dataset/Crashes/crash_info_general.csv', usecols=['CRN', 'DEC_LAT', 'DEC_LONG', 'CRASH_YEAR'])
...
merged_df = merged_df.query('DVR_PED_CONDITION == 1').query('PERSON_TYPE == 1').query('CRASH_YEAR >= 2020').reset_index(drop=True)
...
map_philadelphia.save('heatmap_all_crashes_after2020.html')