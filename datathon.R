# Are there any road properties (intersections, location types, travel directions, road condition), weather conditions or times of day that have a higher incidence of fatal crashes? Can we use predictive modeling to identify these high-risk areas and suggest road repair or redesign to mitigate the risks?
...
vehicle.dat<-read.csv("crash_info_vehicles.csv", header=T)

merged.dat <- merge(gen.dat, vehicle.dat, by = "CRN", all.x = TRUE)
...
merged.dat <- subset(merged.dat, select=c(BELTED_DEATH_COUNT, BICYCLE_DEATH_COUNT, CHLDPAS_DEATH_COUNT, MCYCLE_DEATH_COUNT,
                                          ...
                                          INTERSECTION_RELATED, TRAVEL_DIRECTION, TIME_OF_DAY,
                                          ROAD_CONDITION))
...
merged.dat$TRAVEL_DIRECTION <- as.factor(merged.dat$TRAVEL_DIRECTION)
table(merged.dat$TRAVEL_DIRECTION)
class(merged.dat$TRAVEL_DIRECTION)
...
model1 <- glm(fatal ~ WEATHER1 + WEATHER2 + INTERSECT_TYPE + 
               INTERSECTION_RELATED + TRAVEL_DIRECTION + TIME_OF_DAY + ROAD_CONDITION, 
