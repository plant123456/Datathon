...
crashes = pd.read_csv('dataset/Crashes/crash_info_general.csv', usecols=['CRN', 'DEC_LAT', 'DEC_LONG', 'ILLUMINATION'])
...
merged_df = merged_df.query('DVR_PED_CONDITION == 1').query('PERSON_TYPE == 1').query('ILLUMINATION == 2').reset_index(drop=True)
...
map_philadelphia.save('heatmap_crashes_dark.html')